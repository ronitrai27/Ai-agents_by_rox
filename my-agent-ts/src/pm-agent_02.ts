...
const DB = {
  tasks: [
    ...
    },
    {
      ...
      description: "v2 → v3 zero downtime",
      assignee: "alice",
      status: "todo",
      priority: "high",
      durationDays: 4,
  ...
  issues: [
    ...
    {
      id: "I-003",
      title: "Slow query on user listing",
...
const getTasks = tool(
  (input) => {
    ...
    let rows = DB.tasks;
    if (input.assignee)
      rows = rows.filter((t) => t.assignee === input.assignee);
    if (input.status) rows = rows.filter((t) => t.status === input.status);
    if (input.sprintId)
      rows = rows.filter((t) => t.sprintId === input.sprintId);
    return JSON.stringify(rows.length ? rows : "No tasks found.");
  },
  {
    name: "getTasks",
    description:
      "Query tasks. Optional filters: assignee (name), status (todo/in-progress/done), sprintId.",
    schema: z.object({
...
const getIssues = tool(
  (input) => {
    ...
    if (input.priority)
      rows = rows.filter((i) => i.priority === input.priority);
  ...
  },
  {
    name: "getIssues",
    description:
      "Query issues. Optional filters: status (open/in-progress), priority (critical/high/medium/low), unassigned (bool).",
    schema: z.object({
...
const getSprints = tool(
  ...
  },
  {
    name: "getSprints",
    description:
      "Query sprints. Optional filters: status (active/planned), sprintId.",
    schema: z.object({
...
const assignTask = tool(
  async (input) => {
    ...
    // Execution resumes only when Command({ resume: bool }) is sent.
...
async function runTurn(input: string | Command): Promise<void> {
  ...
  for await (const event of stream) {
    ...
    if (event.event === "on_tool_end") {
      ...
      const preview =
        typeof output === "string" && output.length > 100
          ? output.slice(0, 100) + "…"
          : String(output ?? "");
  ...
  // ── HITL approval ─────────────────────────────────────────
  const payload = (
    interrupted.interrupts[0] as { value: Record<string, unknown> }
  ).value;
  ...
  console.log(
    `    Task  : ${c.bold(String(payload.taskId))} — ${payload.taskTitle}`,
  );
  ...
  // Resume the paused graph. This re-enters the tool after interrupt()
  // with the approved boolean as its return value, then the agent
  // finishes reasoning and we stream that continuation.
  await runTurn(new Command({ resume: approved }));
}
...
async function repl() {
  ...
  console.log(
    c.bold(c.cyan("║       PM Agent — Interactive CLI             ║")),
  );
  console.log(
    c.bold(c.cyan("╚══════════════════════════════════════════════╝")),
  );
  console.log(
    c.dim(
      `  thread: ${config.configurable.thread_id}  |  memory persists across turns`,
    ),
  );
