...
export function ResearchAgentNode({
  ...
  return (
    <div
      className={cn(
        "my-3 p-4 border font-mono rounded-sm transition-all duration-500",
        statusData.is_done
          ? "border-neutral-800 bg-neutral-800 opacity-100"
    ...
    >
      <div className="flex justify-between items-center opacity-40 mb-3 border-b border-neutral-900 pb-2">
        ...
        <div className="flex gap-4">
          {displayTime && (
            <span className="text-neutral-200 font-bold tabular-nums text-[10px]">
              {displayTime}
            </span>
          )}
          <div className="flex items-center gap-2">
            ...
            <span
              className={cn(
                ...
                statusData.is_done ? "text-neutral-200" : "text-emerald-500",
              )}
            >
      ...
      <div className="space-y-1.5">
        <div className="flex items-start gap-3">
          <span className="text-[9px] text-neutral-600 font-black mt-0.5">
            TASK //
          </span>
          <span className="text-xs text-neutral-300 leading-tight uppercase tracking-tight">
            {statusData.status}
          </span>
        </div>
        {statusData.query && (
          <div className="flex items-start gap-3 opacity-80">
            ...
            <span className="text-[10px] text-neutral-500 truncate leading-tight italic">
              {statusData.query}
            </span>
          </div>
        )}
